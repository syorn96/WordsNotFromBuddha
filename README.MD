...
* Open repo terminal and run ```touch .gitignore echo >>> node_modules ```
* ```touch .env ```
* In .env create a variable ENC_KEY 
* EXAMPLE: ```ENC_SECRET=< your secret code >```
* Run ``` npm init ```
...
* Run ```createdb words-not-from-buddha```
